...
export class OffersLiveRepository implements OffersRepository {
  constructor() {}
  ...
  get(offerId: string): Offer {
    const retrievedOffer = this.offers.find(offer => offer.id === offerId);
    if (retrievedOffer === undefined) throw new Error("Offer not found");
